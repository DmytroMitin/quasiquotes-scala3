...
| Definition pattern interpolation | `case dqq"def id(x: Int): Int = $body"`; `case dqq"def choose(x: Int, y: String): String = $body"`; `case dqq"def collect(..$params): Int = $body"` | Private neutral comparison keys plus the frontend-neutral ranked carrier | Yes, caller-owned RHS `Term`; ranked form also returns `Seq[ValDef]` | Public Quotes inspection | `BOUNDED`; one same-spelling transparent-inline selector; static exact one -> `SingleParameterDefinitionPattern`; static structural exact two -> body-only `DefinitionPatternExtractor`; static ranked ordinary clause with 0 through 5 parameters -> `RankedDefinitionPatternExtractor`; dynamic/non-static -> exact-one fallback; original parameter/RHS identity; no `dqq2`/`dqq3`/`dqq4`, rank 3, multiple clauses, or construction-side parameter splicing; typed-Scalameta preserves the same bounded selector split |
