...
private[quasiquotes] object ConstructedTermGeneratedOriginAdapter:
  ...
  private final case class NodePlan(
      kind: NodeKind,
      start: Int,
      end: Int,
      point: Int,
  ...
  def lower(
    ...
    for
      ...
      rendered <- Planner(constructed).render()
      ...
      positioned <- position(raw, rendered.root, source)
      _ <- validatePositioned(positioned, source, rendered.source.length)
    yield new GeneratedOriginTermResult(positioned, rendered.source, source)
  ...
  private[dotty] def validatePositionedForTest(
    ...
    validatePositioned(tree, expectedSource, sourceLength)
  ...
  private final class Planner private (constructed: ConstructedTerm):
    private val builder = new StringBuilder
    private var typedOrdinal = 0

    def render(): Either[ConstructedTermGeneratedOriginError, RenderedPlan] =
      for
        root <- renderTerm(constructed.root)
        _ <- Either.cond(
          typedOrdinal == constructed.ascriptionTypes.size,
          (),
          UnconsumedTypeSidecars(
            typedOrdinal,
            constructed.ascriptionTypes.size
          )
        )
      ...
      shape match
        ...
        case TermShape.Typed(expression, _) =>
          ...
          for
            normalForm <- sidecar
            _ = builder.append('(')
            rawExpression <- renderTerm(expression)
            _ = builder.append("): ")
        ...
        case TermShape.Parenthesized(expression) =>
          val start = builder.length
          builder.append('(')
          renderTerm(expression).map { rawExpression =>
            builder.append(')')
  ...
  private object Planner:
    def apply(constructed: ConstructedTerm): Planner =
      new Planner(constructed)
  ...
  private def validatePositioned(
    ...
    val rootErrors =
      Option.when(
        !tree.span.exists || tree.span.start != 0 || tree.span.end != sourceLength
      )(
        s"root ${tree.getClass.getSimpleName} does not cover generated source 0..$sourceLength"
      ).toVector
    val errors =
