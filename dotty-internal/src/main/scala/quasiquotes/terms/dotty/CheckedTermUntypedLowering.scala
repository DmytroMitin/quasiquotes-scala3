...
/** One completed semantic graph and its once-lowered, verified source-free tree. */
private[dotty] object CheckedTermUntypedLowering:
  import TermUntypedLowering.Failure

  final class Checked private[CheckedTermUntypedLowering] (
      val completed: ConstructedTerm,
      val raw: untpd.Tree
  )
  ...
  def lower(
      term: TermShape
  )(using Context): Either[Failure, Checked] =
    Option(term)
      .toRight(
        Failure(
          "MISSING_INPUT",
      ...
      .flatMap { present =>
        for
          ...
          completed <- contain(ConstructedTerm.fromShape(present), classifyCompletionFailure)
          _ <- validateFacadeLimits(completed.root)
          raw <- contain(ConstructedTermUntypedBackend.lower(completed), classifyBackendFailure)
          _ <- contain(
            CoreTermShapeUntypedLowerer.verifySourceFree(raw),
            problem => invariant("term", problem.message)
          )
          _ <- validateRaw(completed, raw, "term")
        yield new Checked(completed, raw)
      }

  private def contain[E, A](result: Either[E, A], classify: E => Failure): Either[Failure, A] =
    Option(result).toRight(invariant("term", "a private authority returned no result container.")).flatMap {
      case Left(null) => Left(invariant("term", "a private authority returned a null failure."))
      case Left(problem) => Left(classify(problem))
      case Right(value) => Option(value).toRight(invariant("term", "a private authority returned a null success value."))
    }
