...
/** Replaces one exact U014 argument with one bounded U005-style Apply subtree. */
private[quasiquotes] object ExistingUntpdSelectedApplyArgumentApplyRewriter:
  ...
  )(using Context): Either[ExistingUntpdSelectedApplyArgumentApplyRewriteError, Result] =
    try
      for
      ...
      case NonFatal(exception) =>
        Left(
          error(
            "SELECTED_APPLY_ARGUMENT_APPLY_REWRITE_FAILED",
  ...
  private def validateExisting(
      ...
      exactTarget: untpd.DefDef,
      exactArgument: untpd.Tree
  )(using Context): Either[ExistingUntpdSelectedApplyArgumentApplyRewriteError,
  ...
  private def validateReplacement(
      replacement: untpd.Tree
  )(using Context): Either[ExistingUntpdSelectedApplyArgumentApplyRewriteError, untpd.Apply] =
    Option(replacement).toRight(
      error("REPLACEMENT_REQUIRED", "the child-bearing replacement was null.")
    ).flatMap {
      case apply: untpd.Apply =>
        for
          function <- Option(apply.fun).toRight(
            error("REPLACEMENT_FUNCTION_IDENT_REQUIRED", "the replacement function was null.")
          )
          _ <- Either.cond(
            function.isInstanceOf[untpd.Ident],
            (),
            error(
              "REPLACEMENT_FUNCTION_IDENT_REQUIRED",
              s"the replacement function was ${nodeKind(function)}, not direct Ident."
            )
          )
          ...
          _ <- Either.cond(
            ...
            error(
              ...
              s"U015 admits $MinimumReplacementArguments..$MaximumReplacementArguments replacement arguments; found ${arguments.size}."
            )
          )
          ...
          _ <- arguments.zipWithIndex.collectFirst {
            ...
            case None => Right(())
          _ <- validateSourceFree(apply)
        yield apply
      case other =>
        Left(
          error(
            "REPLACEMENT_APPLY_REQUIRED",
            s"the child-bearing replacement was ${nodeKind(other)}, not Apply."
          )
        )
    }
  ...
  private def validateSourceFree(
      apply: untpd.Apply
  )(using Context): Either[ExistingUntpdSelectedApplyArgumentApplyRewriteError, Unit] =
    val nodes = apply +: apply.fun +: apply.args.toVector
  ...
  private def verify(
    ...
    val replacementNodes = result.replacementApply +:
      result.replacementApply.fun +: result.replacementApply.args.toVector
    ...
    Either.cond(
      ...
      error("RECONSTRUCTION_INVARIANT_FAILED",
        "the U015 mixed-provenance result violated its bounded identity/provenance contract.")
    )
  ...
  private def error(
      code: String,
      detail: String
  ): ExistingUntpdSelectedApplyArgumentApplyRewriteError =
    ExistingUntpdSelectedApplyArgumentApplyRewriteError(code, detail)
