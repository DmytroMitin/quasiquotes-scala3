...
class ExistingUntpdMethodBodyRewriteSelectedApplyOriginAdapterTyperRuntimeTest
    extends munit.FunSuite:
  private enum Mode:
    case Valid, MissingService, MissingMember

  test("uniform selected Apply origin survives Typer, emits TASTy, and executes") {
    withCompilation(Mode.Valid) { (driver, reporter, output, _) =>
      ...
      assert(emitted(output).exists(_.endsWith("U013SelectedApplyOrigin.tasty")))
      val loader = new URLClassLoader(Array(output.toUri.toURL), getClass.getClassLoader)
      try
        val moduleClass = loader.loadClass("U013SelectedApplyOriginUse$")
        val module = moduleClass.getField("MODULE$").get(null)
        assertEquals(moduleClass.getMethod("value").invoke(module), Integer.valueOf(21))
      finally loader.close()
    }
  }

  test("missing qualifier is diagnosed at the replaced RHS site") {
    assertInvalid(Mode.MissingService, "missingU013Service")
  }

  test("missing member is diagnosed at the replaced RHS site") {
    assertInvalid(Mode.MissingMember, "missingU013Member")
  }
  ...
  private def assertInvalid(mode: Mode, expectedName: String): Unit =
    withCompilation(mode) { (driver, reporter, output, source) =>
      ...
      assert(driver.originalRhsStart > 0)
      assert(driver.beforeTyperContractValid)
      assert(!emitted(output).exists(_.endsWith("U013SelectedApplyOrigin.tasty")))
    }
  ...
  private def withCompilation(mode: Mode)(
    ...
    val temporary = Files.createTempDirectory("u013-selected-apply-origin-")
    try
      val source = temporary.resolve("U013SelectedApplyOrigin.scala")
      val output = temporary.resolve("classes")
      Files.createDirectories(output)
      Files.writeString(
        source,
        """@deprecated("fixture", "1")
          |class U013SelectedApplyOrigin:
          |  object service:
          |    def invoke(value: Int): Int = value + 1
          |  def change: Int = 2
          |  val opaque: String = "opaque"
          |  def call: Int = change
          |
          |object U013SelectedApplyOriginUse:
          |  def value: Int = new U013SelectedApplyOrigin().change
          |""".stripMargin,
  ...
  private final class AdaptBeforeTyper(mode: Mode, evidence: OriginDriver)
      extends Phase:
    def phaseName: String = "u013ExistingMethodBodySelectedApplyOriginAdaptation"
        ...
        for
          originalRoot <- allTrees(unitTree).collectFirst {
            case value: untpd.TypeDef
                if value.name.toString == "U013SelectedApplyOrigin" => value
          }.toRight("U013 runtime fixture class was not found")
          ...
          }.toRight("U013 runtime fixture target was not found")
          replacement =
          ...
          adapted <- ExistingUntpdMethodBodyRewriteOriginAdapter
            ...
            .left.map(_.message)
        yield (originalRoot, originalTemplate, originalTarget, originalState, structural, adapted)

      prepared match
        case Left(problem) => report.error(problem)
        case Right((originalRoot, originalTemplate, originalTarget, originalState, structural, adapted)) =>
          ...
          evidence.beforeTyperContractValid =
            (Vector[untpd.Tree](
              ...
              positionedReplacementNodes.forall(tree =>
                tree.source == originalTarget.rhs.source && tree.span == originalTarget.rhs.span
              ) &&
          ...
          val transformer = new untpd.UntypedTreeMap:
            override def transform(tree: untpd.Tree)(using Context): untpd.Tree =
              if tree.eq(originalRoot) then adapted.positionedRoot else super.transform(tree)
  ...
  private def emitted(output: Path): Vector[String] =
    ...
    try
      stream.filter(Files.isRegularFile(_)).iterator().asScala.map(_.toString).toVector
