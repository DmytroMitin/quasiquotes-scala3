...
class ExistingUntpdSelectedApplyArgumentWrapSiblingParserOracleTest
    extends munit.FunSuite:
  List(
    ("freshValue", "Ident"),
    ("20", "Number"),
    ("true", "Literal")
  ).foreach { case (siblingSource, siblingKind) =>
    test(s"locks helper(oldArg, $siblingSource) parser topology") {
      withContext {
        ...
        assertEquals(sibling.getClass.getSimpleName, siblingKind)
        ...
        Vector[untpd.Tree](outer, wrapper, function, original, sibling).foreach { tree =>
  ...
  private def parseTarget(source: String)(using outerContext: Context): untpd.DefDef =
    ...
    val unit = CompilationUnit("U019Oracle.scala", source)
