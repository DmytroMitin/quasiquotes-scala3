...
class ExistingUntpdSelectedApplyArgumentSelectedApplyRewriteOriginAdapterTyperRuntimeTest
    extends munit.FunSuite:
  private enum Mode:
    case P0, P1, P2, P3, Valid, MissingMember, MissingQualifier

  List(Mode.P0, Mode.P1, Mode.P2, Mode.P3).foreach { mode =>
  ...
  test("P4 selected-member replacement survives Typer, emits TASTy, and executes") {
    withCompilation(Mode.Valid) { (driver, reporter, output, _) =>
      ...
      assert(emitted(output).exists(_.endsWith("U016SelectedMemberArgumentOrigin.tasty")))
      val loader = new URLClassLoader(Array(output.toUri.toURL), getClass.getClassLoader)
      try
        val moduleClass = loader.loadClass("U016SelectedMemberArgumentOriginUse$")
  ...
  List(
    Mode.MissingMember -> "missingU016Member",
    Mode.MissingQualifier -> "missingU016Qualifier"
  ).foreach { case (mode, expectedName) =>
    test(s"$mode is diagnosed at the exact old argument start") {
  ...
  private def withCompilation(mode: Mode)(
    ...
    val temporary = Files.createTempDirectory("u016-selected-member-argument-origin-")
    try
      val source = temporary.resolve("U016SelectedMemberArgumentOrigin.scala")
      val output = temporary.resolve("classes")
      Files.createDirectories(output)
      Files.writeString(
        source,
        """class U016SelectedMemberArgumentOrigin:
          |  object service:
          |    def invoke(a: Int, b: Int): Int = a + b
          |  object helper:
          |    def make(x: Int): Int = x + 1
          |  val oldArg: Int = 1
          |  val keptArg: Int = 1
          |  def keep: Int = 1
          ...
          |
          |object U016SelectedMemberArgumentOriginUse:
          |  def value: Int = new U016SelectedMemberArgumentOrigin().change
          |""".stripMargin,
  ...
  private final class RewriteBeforeTyper(mode: Mode, evidence: OriginDriver) extends Phase:
    def phaseName: String = "u016SelectedMemberArgumentOriginAdaptation"
        ...
        for
          root <- allTrees(unitTree).collectFirst {
            case value: untpd.TypeDef
                if value.name.toString == "U016SelectedMemberArgumentOrigin" => value
          }.toRight("U016 runtime fixture class was not found")
          ...
          }.toRight("U016 runtime target was not found")
          outer <- target.rhs match
            case value: untpd.Apply => Right(value)
            case other => Left(s"fixture rhs was ${other.getClass.getSimpleName}")
          exactArgument <- outer.args.headOption.toRight("U016 runtime argument missing")
          replacement =
            given SourceFile = NoSource
            val qualifierName = mode match
              case Mode.MissingQualifier => "missingU016Qualifier"
              case _ => "helper"
            val memberName = mode match
              case Mode.MissingMember => "missingU016Member"
              case _ => "make"
            untpd.Apply(
              untpd.Select(untpd.Ident(termName(qualifierName)), termName(memberName)),
              untpd.Number("20", untpd.NumberKind.Whole(10)) :: Nil
            )
          originalState = allTrees(root).map(tree => (tree, tree.source, tree.span, tree.symbol))
          structural <- ExistingUntpdSelectedApplyArgumentSelectedApplyRewriter
            .rewrite(root, target, exactArgument, replacement).left.map(_.message)
          adapted <-
            if mode == Mode.Valid || mode == Mode.MissingMember || mode == Mode.MissingQualifier then
              ExistingUntpdSelectedApplyArgumentSelectedApplyRewriteOriginAdapter
      ...
      prepared match
        ...
        case Right((root, template, target, outer, exactArgument, replacement,
          ...
          val finalRoot = adaptedOption match
            case Some(adapted) =>
              val structuralSelection = replacement.fun.asInstanceOf[untpd.Select]
              val structuralNodes = Vector[untpd.Tree](
                ...
                replacement,
                structuralSelection,
                structuralSelection.qualifier
              ) ++ replacement.args
              val positionedNodes = Vector[untpd.Tree](
                adapted.positionedReplacement,
              ...
              evidence.beforeTyperContractValid =
                structuralNodes.forall(tree =>
                  ...
                  adapted.positionedReplacement.fun.eq(adapted.positionedReplacementSelection) &&
                  adapted.positionedReplacementSelection.qualifier
                  ...
                  !adapted.positionedReplacementSelection.eq(structuralSelection) &&
                  !adapted.positionedReplacementQualifier.eq(structuralSelection.qualifier) &&
    ...
    private def strategyRoot(
      ...
      if mode == Mode.P0 then structural.rebuiltRoot
        ...
        val originalSelection = structural.replacementApply.fun.asInstanceOf[untpd.Select]
        val replacement = mode match
          case Mode.P1 => structural.replacementApply
          case Mode.P2 =>
            untpd.Apply(originalSelection, structural.replacementApply.args)
              .cloneIn(structural.originalArgument.source)
              .withSpan(structural.originalArgument.span)
          case Mode.P3 =>
            val positionedSelection = untpd
              .Select(originalSelection.qualifier, originalSelection.name)
              .cloneIn(structural.originalArgument.source)
              .withSpan(structural.originalArgument.span)
            untpd.Apply(positionedSelection, structural.replacementApply.args)
              .cloneIn(structural.originalArgument.source)
              .withSpan(structural.originalArgument.span)
          case other => throw new IllegalArgumentException(s"unexpected strategy mode: $other")
  ...
  private def compilationClasspath: String =
    Vector(
      ...
      classOf[ExistingUntpdSelectedApplyArgumentSelectedApplyRewriteOriginAdapter.type],
      getClass
    ).flatMap(value =>
