...
class ExistingUntpdTwoParameterMethodResultTypeRewriterTyperRuntimeTest
    extends munit.FunSuite:
  test("U035 no-rewrite baseline fails because AnyVal does not expose integer addition") {
    val temporary = Files.createTempDirectory("u035-result-type-baseline-")
    try
      ...
      assert(
        reporter.allErrors.exists(problem =>
          problem.message.contains("AnyVal") && problem.message.contains("+")
        ),
        clues(reporter.allErrors)
      )
    finally deleteRecursively(temporary)
  }

  test("U035 result-type rewrite survives Typer emits TASTy and runs at 43") {
    val temporary = Files.createTempDirectory("u035-result-type-rewrite-")
    try
      val source = writeFixture(temporary)
      val output = temporary.resolve("classes")
      Files.createDirectories(output)
      ...
      assert(emitted(output).exists(_.endsWith("U035ResultTypeRewriteRuntime.tasty")))
      val loader = new URLClassLoader(Array(output.toUri.toURL), getClass.getClassLoader)
      try
        val moduleClass = loader.loadClass("U035ResultTypeRewriteRuntimeUse$")
        val module = moduleClass.getField("MODULE$").get(null)
        assertEquals(moduleClass.getMethod("value").invoke(module), Integer.valueOf(43))
  ...
  private final class RewriteBeforeTyper(evidence: RewriteDriver) extends Phase:
    def phaseName: String = "u035TwoParameterMethodResultTypeRewrite"
    ...
    protected def run(using Context): Unit =
        ...
        for
          originalRoot <- allTrees(unitTree).collectFirst {
            case value: untpd.TypeDef
                if value.name.toString == "U035ResultTypeRewriteRuntime" => value
          }.toRight("U035 runtime fixture class was not found")
          ...
          rewritten <- ExistingUntpdTwoParameterMethodResultTypeRewriter
      ...
      prepared match
        ...
        case Right((originalRoot, originalTemplate, view, originalState, rewritten)) =>
          val first = rewritten.positionedMethod.paramss.head(0).asInstanceOf[untpd.ValDef]
          val second = rewritten.positionedMethod.paramss.head(1).asInstanceOf[untpd.ValDef]
  ...
  private def writeFixture(temporary: Path): Path =
    val source = temporary.resolve("U035ResultTypeRewriteRuntime.scala")
    Files.writeString(
      source,
      """class U035ResultTypeRewriteRuntime:
        |  val before: Int = 1
        |  def sum(x: Int, y: Int): AnyVal = x + y
        |  type After = String
        |
        |object U035ResultTypeRewriteRuntimeUse:
        |  def value: Int = new U035ResultTypeRewriteRuntime().sum(20, 22) + 1
        |""".stripMargin,
  ...
  private def compilationClasspath: String =
    Vector(
      ...
      classOf[ExistingUntpdTwoParameterMethodResultTypeRewriter.type],
      getClass
    ).flatMap(value =>
