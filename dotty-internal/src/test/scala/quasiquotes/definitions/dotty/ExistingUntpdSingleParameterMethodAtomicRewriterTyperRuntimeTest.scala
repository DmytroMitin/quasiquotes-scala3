...
class ExistingUntpdSingleParameterMethodAtomicRewriterTyperRuntimeTest
    extends munit.FunSuite:
  test("U032 atomically makes parameter result and selected-Apply RHS bind before Typer") {
    val temporary = Files.createTempDirectory("u032-atomic-rewrite-")
    try
      val source = temporary.resolve("U032AtomicRewriteRuntime.scala")
      val output = temporary.resolve("classes")
      Files.createDirectories(output)
      Files.writeString(
        source,
        """class U032AtomicRewriteRuntime:
          |  val before: Int = 1
          |  def bump(x: AnyVal): AnyVal = x
          |  type After = String
          |
          |object U032AtomicRewriteRuntimeUse:
          |  def value: Int = new U032AtomicRewriteRuntime().bump(-7) + 1
          |""".stripMargin,
      ...
      assert(emitted(output).exists(_.endsWith("U032AtomicRewriteRuntime.tasty")))
      val loader = new URLClassLoader(Array(output.toUri.toURL), getClass.getClassLoader)
      try
        val moduleClass = loader.loadClass("U032AtomicRewriteRuntimeUse$")
  ...
  private final class RewriteBeforeTyper(evidence: RewriteDriver) extends Phase:
    def phaseName: String = "u032AtomicSingleParameterMethodRewrite"
    ...
    protected def run(using Context): Unit =
        ...
        for
          originalRoot <- allTrees(unitTree).collectFirst {
            case value: untpd.TypeDef
                if value.name.toString == "U032AtomicRewriteRuntime" => value
          }.toRight("U032 runtime fixture class was not found")
          ...
          rewritten <- ExistingUntpdSingleParameterMethodAtomicRewriter
            .rewrite(
              view,
      ...
      prepared match
        ...
        case Right((originalRoot, originalTemplate, view, originalState, rewritten)) =>
          val parameter = rewritten.parameterRewrite.positionedParameter
          val rhs = rewritten.rhsRewrite.positionedResult.positionedReplacement
          val originalUntouched = originalTemplate.body.filterNot(_.eq(view.method))
          val rewrittenUntouched = rewritten.positionedTemplate.body.filterNot(_.eq(rewritten.positionedMethod))
          evidence.beforeTyperContractValid =
            !parameter.eq(view.parameter) &&
              !parameter.tpt.eq(view.parameterType) &&
              !rewritten.resultRewrite.positionedResultType.eq(view.resultType) &&
              !rhs.eq(view.rhs) &&
              rewritten.positionedMethod.paramss.head.head.eq(parameter) &&
              rewritten.positionedMethod.tpt.eq(rewritten.resultRewrite.positionedResultType) &&
              rewritten.positionedMethod.rhs.eq(rhs) &&
  ...
  private def compilationClasspath: String =
    Vector(
      ...
      classOf[ExistingUntpdSingleParameterMethodAtomicRewriter.type],
      getClass
    ).flatMap(value =>
