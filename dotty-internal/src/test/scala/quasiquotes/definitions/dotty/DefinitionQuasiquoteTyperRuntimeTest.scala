...
class DefinitionQuasiquoteTyperRuntimeTest extends munit.FunSuite:
  test("completed core output survives generated-origin insertion typer TASTy class emission and runtime use") {
    val constructed =
      ConstructedDefinition
        .parameterlessDef(
          DefinitionName.plain("generatedMethod").toOption.get,
          quasiquotes.types.TypeNormalForm.STypeIdent("String"),
          quasiquotes.terms.ConstructedTerm
            .fromShape(
              quasiquotes.parser.TermShape.Infix(
                quasiquotes.parser.TermShape.Literal("\"phase52:\""),
                "+",
                quasiquotes.parser.TermShape.Literal("\"runtime\"")
              )
            )
            .toOption
            .get
