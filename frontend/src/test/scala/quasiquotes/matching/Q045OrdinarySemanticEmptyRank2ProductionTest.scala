...
final class Q045OrdinaryAnnotation extends StaticAnnotation

abstract class Q045OrdinaryInlineFixture:
  inline def inlineMixed(x: Int): Int = x
  transparent inline def transparentMixed(x: Int): Int = x
  def deferredMixed(x: Int): Int

final class Q045OrdinarySemanticEmptyRank2ProductionTest
  ...
  test("Q044 captures and Q045Ordinary rejects every semantically nonempty modifier family"):
    import quasiquotes.matching.DefinitionPattern.dqq
  ...
  private def fixtureDefinitions(using q: Quotes): Map[String, q.reflect.DefDef] =
    ...
    traversal.traverseTree('{
      ...
      class Fixture extends Base:
        ...
        @Q045OrdinaryAnnotation def annotatedMixed(x: Int): Int = x
    ...
    def inlineDefinition(name: String): DefDef =
      val target =
        TypeRepr.of[Q045OrdinaryInlineFixture].typeSymbol.declaredMethod(name).head.tree.asInstanceOf[DefDef]
