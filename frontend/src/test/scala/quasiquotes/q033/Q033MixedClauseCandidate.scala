...
object Q033MixedClauseCandidateFactory:
...
object Q033DefinitionPatternMacros:
  def standard(
    ...
    parts match
      ...
      case None =>
        quotes.reflect.report.errorAndAbort(
          s"Q033 $frontend mixed-clause Definition template must be statically known.",
          context
        )
  ...
  private def diagnostic(frontend: String, parts: List[String]): String =
    ...
    val detail =
      if literal.contains("...") then "rank-3 capture is outside Q033"
      else if parts.size != 7 then "exactly six ordered captures are required"
      else if !literal.contains("(using") then "the second clause must be an explicit named using clause"
      else "only `$mods def $name(..$params)(using ..$usingParams): $result = $body` is selected"
    s"Invalid Q033 $frontend dqq Definition template: $detail."

object Q033StandardDefinitionPattern:
  extension (inline context: StringContext)
    transparent inline def dqq(using q: Quotes) =
      ${ Q033DefinitionPatternMacros.standard('context, 'q) }
