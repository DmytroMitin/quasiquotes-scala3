...
object QuasiTypeSpliceExamples:
  ...
  private def spliceSummaryImpl(
    ...
    val summary =
      for
        binding <- TypeNormalFormSource.fromSource(bindingSource.valueOrAbort)
  ...
  private def spliceSummaryImpl(
    ...
    val summary =
      for
        firstBinding <- TypeNormalFormSource.fromSource(firstBindingSource.valueOrAbort)
        secondBinding <- TypeNormalFormSource.fromSource(secondBindingSource.valueOrAbort)
