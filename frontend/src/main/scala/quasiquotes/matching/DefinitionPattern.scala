...
object DefinitionPattern:
  ...
  private[matching] def capturedModifiersNameOrdinaryParameterSequenceCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedModifiersNameOrdinaryParameterSequenceCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedModifiersNameOrdinaryParamsResult
      case Some(_) =>
        abort(
          "expected exactly `$mods def $name(..$params): $result = $body` with five captures in semantic-modifiers, semantic-name, complete-ordinary-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")

  private[matching] def capturedNameOrdinaryParameterSequenceCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedNameOrdinaryParameterSequenceCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedNameOrdinaryParamsResult
      case Some(_) =>
        abort(
          "expected exactly `def $name(..$params): $result = $body` with four captures in semantic-name, complete-ordinary-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")

  private[matching] def capturedModifiersNameNamedUsingParameterSequenceCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedModifiersNameNamedUsingParameterSequenceCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedModifiersNameNamedUsingParamsResult
      case Some(_) =>
        abort(
          "expected exactly `$mods def $name(using ..$params): $result = $body` with five captures in semantic-modifiers, semantic-name, complete-named-using-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")
  ...
  private[matching] def capturedNameMixedOrdinaryNamedUsingParameterSequencesCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedNameMixedOrdinaryNamedUsingParameterSequencesCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedNameMixedOrdinaryNamedUsingParamsResult
      case Some(_) =>
        abort(
          "expected exactly `def $name(..$params)(using ..$usingParams): $result = $body` with five captures in semantic-name, complete-ordinary-parameters, complete-named-using-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")
  ...
  private[matching] def capturedNameMixedOrdinaryScala2ImplicitParameterSequencesCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedNameMixedOrdinaryScala2ImplicitParameterSequencesCapturedResult(
            values
          ) =>
        RankedDefinitionPatternExtractorFactory.capturedNameMixedOrdinaryScala2ImplicitParamsResult
      case Some(_) =>
        abort(
          "expected exactly `def $name(..$params)(implicit ..$implicitParams): $result = $body` with five captures in semantic-name, complete-ordinary-parameters, complete-Scala-2-implicit-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")
  ...
  private[matching] def capturedNameNamedUsingParameterSequenceCapturedResultExtractor(
    ...
    parts match
      case Some(values)
          if isExactCapturedNameNamedUsingParameterSequenceCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedNameNamedUsingParamsResult
      case Some(_) =>
        abort(
          "expected exactly `def $name(using ..$params): $result = $body` with four captures in semantic-name, complete-named-using-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")

  private[matching] def capturedModifiersNameScala2ImplicitParameterSequenceCapturedResultExtractor(
      sc: StringContext
  )(using q: Quotes): RankedDefinitionPatternExtractor[
    q.reflect.DefDef,
    (
    ...
    parts match
      case Some(values)
          if isExactCapturedModifiersNameScala2ImplicitParameterSequenceCapturedResult(values) =>
        RankedDefinitionPatternExtractorFactory.capturedModifiersNameScala2ImplicitParamsResult
      case Some(_) =>
        abort(
          "expected exactly `$mods def $name(implicit ..$params): $result = $body` with five captures in semantic-modifiers, semantic-name, complete-Scala-2-implicit-parameters, semantic-result, and complete-body order"
        )
      case None => abort("StringContext must not be null.")
  ...
  private[matching] def classifyStaticParts(
    ...
    else if isExactCapturedNameMixedOrdinaryNamedUsingParameterSequencesCapturedResult(parts) then
      Right(StaticPatternKind.CapturedNameMixedOrdinaryNamedUsingParameterSequencesCapturedResult)
    ...
    then
      Right(
        StaticPatternKind.CapturedNameMixedOrdinaryScala2ImplicitParameterSequencesCapturedResult
      )
    ...
    else if isExactCapturedModifiersNameOrdinaryParameterSequenceCapturedResult(parts) then
      Right(StaticPatternKind.CapturedModifiersNameOrdinaryParameterSequenceCapturedResult)

    else if isExactCapturedNameOrdinaryParameterSequenceCapturedResult(parts) then
      Right(StaticPatternKind.CapturedNameOrdinaryParameterSequenceCapturedResult)
