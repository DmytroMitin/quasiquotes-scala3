...
private[quasiquotes] object TermQ3ParityMatrix:
  ...
  val rows: Vector[Row] = Vector(
    ...
    Row("p2-single-typed-local-immutable-val", "{ val x: Int = 1; x }", HYBRID_SCALAMETA_SUPPORTED, "shared binder identity, typed ValDef lowering, and alpha-aware matching"),
    Row("remaining-local-definitions", "{ val x = 1; x }; { var x: Int = 1; x }; { def x = 1; x }", NOT_A_PUBLIC_TERM_CASE, "inferred, mutable, multiple, pattern, lazy, and method definitions remain excluded"),
