package quasiquotes.hybrid.q017
...
import _root_.quasiquotes.q017.{Q017CandidateAFactory, Q017CandidateBFactory}

object Q017ScalametaDefinitionParamssPatternMacro:
  def candidateA(
    ...
    parts match
      ...
      case None =>
        quotes.reflect.report.errorAndAbort(
          "Q017 typed-Scalameta rank-3 Definition template must be statically known.",
          context
        )
  ...
  private def structuralSentinelCheck(parts: List[String]): Either[String, Unit] =
    val List(prefix, between, suffix) = parts: @unchecked
    val literal = parts.mkString("<capture>")
    val first = freshName("q017First", literal)
    val second = freshName("q017Second", literal)
    val body = freshName("q017Body", literal)
    val markerOffset = prefix.lastIndexOf("...")
    val source =
      prefix.substring(0, markerOffset) +
    ...
    parseDefinition(source).flatMap { definition =>
      ...
      valid.toRight("Scalameta sentinel projection did not preserve the exact rank-3 Definition topology")
    }
  ...
  private def abort(context: Expr[StringContext], detail: String)(using Quotes): Nothing =
    quotes.reflect.report.errorAndAbort(
      s"Invalid Q017 typed-Scalameta dqq Definition template: $detail.",
      context
    )

object Q017CandidateAScalametaPattern:
  extension (inline context: StringContext)
    transparent inline def dqq(using q: Quotes) =
      ${ Q017ScalametaDefinitionParamssPatternMacro.candidateA('context, 'q) }

object Q017CandidateBScalametaPattern:
  extension (inline context: StringContext)
    transparent inline def dqq(using q: Quotes) =
      ${ Q017ScalametaDefinitionParamssPatternMacro.candidateB('context, 'q) }
