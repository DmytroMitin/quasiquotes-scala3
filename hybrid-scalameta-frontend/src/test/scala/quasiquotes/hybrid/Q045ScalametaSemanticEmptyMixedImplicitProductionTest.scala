...
final class Q045ScalametaAnnotation extends StaticAnnotation

abstract class Q045ScalametaInlineFixture:
...
final class Q045ScalametaSemanticEmptyMixedImplicitProductionTest
    extends munit.FunSuite:
  test("external packages receive the exact Q045 production capture type"):
    val _ = external.consumer.Q045ExternalScalametaMixedOrdinaryScala2ImplicitConsumer
  ...
  test("standard Q045 selector admits only the exact modifier-omitted static grammar"):
    ...
    inline def patternMessages(inline pattern: String): List[String] =
      messages(
        """import scala.quoted.*
import scala.language.experimental.erasedDefinitions; import quasiquotes.scalameta.ScalametaQuasiPattern.dqq
...
import scala.language.experimental.erasedDefinitions; import quasiquotes.matching.{DefinitionPattern, RankedDefinitionPatternExtractor}
         ...
         ] = quasiquotes.scalameta.ScalametaQuasiPattern.dqq(StringContext("", " def ", "(..", ")(implicit ..", "): ", " = ", ""))(using q)"""
    )
  ...
  private def fixtureDefinitions(using q: Quotes): Map[String, q.reflect.DefDef] =
    ...
    traversal.traverseTree('{
      ...
      class Fixture extends Base:
        ...
        @Q045ScalametaAnnotation def annotatedMixed(x: Int)(implicit ordering: Ordering[Int]): Int = x
    ...
    def inlineDefinition(name: String): DefDef =
      val target =
        TypeRepr.of[Q045ScalametaInlineFixture].typeSymbol.declaredMethod(name).head.tree.asInstanceOf[DefDef]
