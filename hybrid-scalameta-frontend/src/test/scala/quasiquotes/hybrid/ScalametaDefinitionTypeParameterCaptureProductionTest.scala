...
final class ScalametaDefinitionTypeParameterCaptureProductionTest extends munit.FunSuite:
  test("external packages receive exact direct and umbrella typed-Scalameta five-capture types"):
    val _ = external.consumer.Q022ExternalScalametaDefinitionTypeParameterCaptureConsumer

  test("typed-Scalameta production matching preserves the complete Q022 success matrix"):
    ...
    val rows = withQuotes:
      ...
      val extractor = quasiquotes.scalameta.ScalametaQuasiPattern.dqq(
        StringContext("def ", "[..", "](...", "): ", " = ", "")
      )(using q)
      ...
      targets.map { (expectedName, expectedTypeNames, expectedTermCounts, target) =>
        ...
        val dependentIdentity =
          if target.name != "dependent" then true
            ...
            arguments.size == 1 && arguments.head.typeSymbol == typeClause.params.head.symbol
        (
          expectedName,
          expectedTypeNames,
          expectedTermCounts,
  ...
  test("typed-Scalameta Q022 matching rejects absent contextual reordered and non-method targets"):
    given Compiler = Compiler.make(getClass.getClassLoader)
    val rows = withQuotes:
      val q = summon[Quotes]
      import q.reflect.*
      ...
      List(
        ...
        "constructor" -> definition('{ class Sample[A](value: A); () }, "<init>"),
        "extension" -> definition('{ extension [A](value: A) def expanded[B](other: B): A = value; () }, "expanded"),
        ...
        "given" -> definition('{ given provided[A](using value: A): A = value; () }, "provided"),
        "null" -> null.asInstanceOf[DefDef]
      ).map((label, target) => label -> extractor.unapply(target).isEmpty)

    rows.foreach(row => assert(row._2, row))

  test("typed-Scalameta production selector admits only the exact static six-part layout"):
  ...
  test("typed-Scalameta five-capture dynamic selection retains exact-one fallback"):
    val errors = typeCheckErrors(
      """import scala.quoted.*; import quasiquotes.matching.RankedDefinitionPatternExtractor
