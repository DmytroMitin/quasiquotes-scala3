...
final class ScalametaDefinitionNameResultCaptureTest extends munit.FunSuite:
  test("external packages receive exact direct and umbrella typed-Scalameta capture types"):
    val _ = external.consumer.Q020ExternalScalametaDefinitionCaptureConsumer

  test("typed-Scalameta production capture preserves the complete success matrix"):
  ...
  test("typed-Scalameta production capture rejects the retained target boundary"):
    given Compiler = Compiler.make(getClass.getClassLoader)
    val rows = withQuotes:
      val q = summon[Quotes]
      import q.reflect.*
  ...
  test("typed-Scalameta selector admits only the exact static four-capture layout"):
  ...
  test("typed-Scalameta dynamic selection retains the historical exact-one fallback"):
